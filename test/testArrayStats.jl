## --- ArrayStats.jl
    ...
    @test nanmedian(A, dims=1) == median(A, dims=1)
    @test nanmedian(A, dims=2) == median(A, dims=2)
    @test nanminimum(A, dims=1) |> vec == [1,101,201]
    @test nanminimum(A, dims=2) |> vec == 1:100
    @test nanmaximum(A, dims=1) |> vec == [100,200,300]
    @test nanmaximum(A, dims=2) |> vec == 201:300
    @test nanmean(A, dims=1) |> vec == [50.5, 150.5, 250.5]
    @test nanmean(A, dims=2) |> vec == 101:200
    @test nanmean(A, ones(size(A)), dims=1) |> vec == [50.5, 150.5, 250.5] # weighted
    @test nanmean(A, ones(size(A)), dims=2) |> vec == 101:200 # weighted
    @test nanstd(A, dims=1) |> vec ≈ fill(29.011491975882016,3)
    @test nanstd(A, dims=2) |> vec ≈ fill(100,100)
    @test nanstd(A, ones(size(A)), dims=1) |> vec ≈ fill(29.011491975882016,3) # weighted
    @test nanstd(A, ones(size(A)), dims=2) |> vec ≈ fill(100,100) # weighted
    @test nanmedian(A, dims=1) |> vec == [50.5, 150.5, 250.5]
    @test nanmedian(A, dims=2) |> vec == 101:200
    @test pctile(A, 10, dims=1) |> vec == [10.9, 110.9, 210.9]
    @test pctile(A, 10, dims=2) |> vec ≈ 21:120
    @test nanmad(A, dims=1) |> vec == [25.0 25.0 25.0]
    @test nanmad(A, dims=2) |> vec == fill(100.0,100,1)
    @test nanaad(A, dims=1) |> vec == [25.0 25.0 25.0]
    @test nanaad(A, dims=2) |> vec ≈ fill(200/3,100,1)
